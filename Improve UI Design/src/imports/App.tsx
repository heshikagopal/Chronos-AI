...
export function App() {
  ...
  useEffect(() => {
    localStorage.setItem('chronos_preferences', JSON.stringify({ ...preferences, energyPeak }));
  ...
  useEffect(() => {
    localStorage.setItem('chronos_blocks', JSON.stringify(blocks));
  }, [blocks]);

  useEffect(() => {
    localStorage.setItem('chronos_distractions', JSON.stringify(distractions));
  }, [distractions]);
  ...
  const handleGenerateSchedule = () => {
    ...
    if (result.blocks.length > 0) {
      const firstWorkBlock = result.blocks.find(b => !b.isBreak);
      if (firstWorkBlock) {
        setActiveFocusBlock(firstWorkBlock);
      }
    }
  ...
  const handleCompleteBlock = (blockId: string, actualMinutes: number) => {
    ...
    if (remaining.length > 0) {
      setActiveFocusBlock(remaining[0]);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
      ...
      <footer className="border-t border-slate-900 py-4 text-center text-xs text-slate-500">
        ChronosAI Daily Planner • Intelligent Priority Time-Blocking & Focus Execution
      </footer>
